...
from bayesian_studio.engine.state_db import get_state_at, load_state_timelines
            ...
            with slot.container():
                ...
                for eid in sub_entity_ids:
                    ...
                    if _is_binary_entity(eid, timelines):
                        ...
                        if fig.data:
                            st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False},
                                            key=f"obs_chart_tpl_sub_num_{i}_{eid}")
        else:
            results_col = [r["obs_results"][i] for r in rows if i < len(r["obs_results"])]
            if results_col:
                fig = _build_binary_chart(results_col, dt_vals, end_dt)
...
if period == "Custom":
    ...
    start_ts = (now - _PERIODS[period]).timestamp()
...
if is_dirty:
    ...
    if source.kind == "yaml":
        ...
        if not st.session_state.get(confirm_key):
            ...
            st.warning("This will overwrite your YAML config file. Proceed?")
            ...
            with col_yes:
                if st.button("Confirm save", type="primary", key=f"confirm_yes_{selected}"):
                    ...
                    st.session_state[_backfill_key] = "pending"
                    st.rerun()
            with col_no:
...
# --- Post-save banner + backfill ---
if _save_ok_key in st.session_state:
    st.success(
        f"{st.session_state.pop(_save_ok_key)} "
        "Visual trace recomputed with updated parameters."
    )

_bf_state = st.session_state.get(_backfill_key)
if _bf_state is not None:
    from bayesian_studio.engine.backfill import get_sqlite_path, run_backfill
    _db_path = get_sqlite_path(CONFIG_DIR)
    if _db_path:
        if _bf_state == "pending":
            if st.button("Backfill recorder history", key=f"_bf_btn_{selected}"):
                _bf_lat, _bf_lon = _get_location()
                _prog = st.progress(0.0, text="Writing rows to recorder DB…")
                try:
                    _bf_res = run_backfill(
                        entity_id=selected,
                        working=working,
                        timelines=timelines,
                        start_ts=start_ts,
                    ...
                    _prog.empty()
                    st.session_state[_backfill_key] = _bf_res
                    st.rerun()
                except Exception as _bf_err:
                    _prog.empty()
                    st.session_state[_backfill_key] = {"error": str(_bf_err)}
                    st.rerun()
        elif isinstance(_bf_state, dict) and "rows_written" in _bf_state:
            st.success(
                f"Backfill complete: {_bf_state['rows_written']} rows written "
